...
const Markdown = forwardRef<MarkdownRef, MarkdownProps>(({ interval = 30, isClosePrettyTyped = false }, ref) => {
        ...
        if (char.content === '\n\n') {
          if (currentParagraph) {
            setStableParagraphs((prev) => {
              ...
              newParagraphs.push({ content: '', isTyped: false, type: 'br', answerType: char.answerType });
  ...
  useImperativeHandle(ref, () => ({
    push: (content: string, answerType: AnswerType) => {
      ...
      console.log('charsGroup', charsGroup);
    ...
    return (
      <div className={`ds-typed-${answerType}`}>
        {paragraphs.map((paragraph, index) => {
          ...
          return (
            <ReactMarkdown key={index} >{paragraph.content || ''}</ReactMarkdown>
          );
        })}
        {currentParagraph?.answerType === answerType && (
          <ReactMarkdown key={currentParagraph.content} >{currentParagraph.content || ''}</ReactMarkdown>
        )}
      </div>
    );
  };

  return <div className="ds-typed">
